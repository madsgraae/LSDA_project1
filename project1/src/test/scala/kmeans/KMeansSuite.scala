...
/**
  *   Credit:
  *   All test cases are based on Coursera course "Parallel programming" 
  *   by Viktor Kuncak & Aleksandar Prokoper
  **/
...
class KMeansSuite extends FunSuite {
  ...
  test("'classify' should work for empty 'points' and 'means' == GenSeq(Point(1,1,1))") {
    val points: GenSeq[Point] = IndexedSeq()
    val mean = new Point(Seq(1, 1, 1))
    val means: GenSeq[Point] = IndexedSeq(mean)
    val expected = GenMap[Point, GenSeq[Point]]((mean, GenSeq()))
    checkClassify(points, means, expected)
  }

  test("'classify' should work for \n 'points' == GenSeq((1, 1, 0), (1, -1, 0), (-1, 1, 0), (-1, -1, 0)) \n and 'means' == GenSeq((0, 0, 0))") {
    val p1 = new Point(Seq(1, 1, 0))
    val p2 = new Point(Seq(1, -1, 0))
    val p3 = new Point(Seq(-1, 1, 0))
    val p4 = new Point(Seq(-1, -1, 0))
    val points: GenSeq[Point] = IndexedSeq(p1, p2, p3, p4)
    val mean = new Point(Seq(0, 0, 0))
    val means: GenSeq[Point] = IndexedSeq(mean)
    val expected = GenMap((mean, GenSeq(p1, p2, p3, p4)))
    checkClassify(points, means, expected)
  }

  test("'classify' should work for \n  'points' == GenSeq((1, 1, 0), (1, -1, 0), (-1, 1, 0), (-1, -1, 0)) \n and 'means' == GenSeq((1, 0, 0), (-1, 0, 0))") {
    val p1 = new Point(Seq(1, 1, 0))
    val p2 = new Point(Seq(1, -1, 0))
    val p3 = new Point(Seq(-1, 1, 0))
    val p4 = new Point(Seq(-1, -1, 0))
    val points: GenSeq[Point] = IndexedSeq(p1, p2, p3, p4)
    val mean1 = new Point(Seq(1, 0, 0))
    val mean2 = new Point(Seq(-1, 0, 0))
  ...
  test("'classify with data parallelism should work for \n empty 'points' and empty 'means'") {
    val points: GenSeq[Point] = IndexedSeq()
    val means: GenSeq[Point] = IndexedSeq()
    val expected = GenMap[Point,GenSeq[Point]]()
